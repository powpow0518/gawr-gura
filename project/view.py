...
class singleFrame(Frame):  # 繼承Frame類
    ...
    def choose(self):  # 點選後需要做的事情, 取得所有資料
        ...
        # 下載該下載的info
        Globals.selected_id, Globals.selected_subs,  Globals.selected_desc = yt.get_all_for_single(
            Globals.id)
...
class pluralFrame(Frame):  # 繼承Frame類
    ...
    def add2thelist(self):
        ...
        Globals.plural_searched_list.append(
            {'id': id_, 'name': name, 'subs': subs, 'video': videoCount, 'view': viewCount, 'code': Globals.plural_searcd_code})
    ...
    def deletethelist(self):
        ...
        # 先暫時幫我的list給編號
        searched_list_by_code = sorted(
            Globals.plural_searched_list, key=lambda x: x['code'])
        # print("bf update:", searched_list_by_code)  #印出編號更新後

        # 刪掉所選取的項目
        for i in range(len(searched_list_by_code)):
            # 我的編號只要跟index一樣 => 刪掉整個資訊(dict)
            if searched_list_by_code[i]['code'] == selected_index:
